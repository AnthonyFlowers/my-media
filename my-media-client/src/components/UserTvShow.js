...
export default function UserTvShow({ uts, handleDelete }) {
  ...
  const debounceHandleUpdateShow = useCallback(
    debounce((nextShow) => {
      updateUserTvShow(nextShow).catch(setErrs);
    }, 1000),
    []
  );
  ...
  function unwatch() {
    if (
      userTvShow.season === 0 &&
      userTvShow.episode === 0 &&
      userTvShow.watchCount === 0
    ) {
      return;
    }
    const nextTvShow = { ...userTvShow };
    ...
    <div className="media-card-lg group">
      ...
      <div className="details">
        <p className="attribute">
          Season:{" "}
          <button value="-1" name="season" onClick={handleChange}>
            &lt;
          </button>{" "}
          {userTvShow.season}{" "}
          <button value="1" name="season" onClick={handleChange}>
        ...
        <p className="attribute">
          Episode:{" "}
          <button value="-1" name="episode" onClick={handleChange}>
            &lt;
          </button>{" "}
          {userTvShow.episode}{" "}
          <button value="1" name="episode" onClick={handleChange}>
        ...
        <p className="attribute">
          Watch Count:{" "}
          <button value="-1" name="watchCount" onClick={handleChange}>
            &lt;
          </button>{" "}
          {userTvShow.watchCount}{" "}
          <button value="1" name="watchCount" onClick={handleChange}>
            &gt;
          </button>
        </p>
        ...
        <p className="overview group-hover:h-auto">
          Overview: {userTvShow.tvShow.overview}
        </p>
